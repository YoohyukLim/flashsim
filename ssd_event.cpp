...
/* Yoohyuk Lim
 * Event initializer ver.Multistream */
/* see "enum event_type" in ssd.h for details on event types */
Event::Event(enum event_type type, ulong logical_address, uint size, double start_time, uint streamID):
...
//Yoohyuk Lim
uint Event::get_streamID(void) const
{
    return streamID;
}
...
void Event::print(FILE *stream)
{
    ...
    //Yoohyuk Lim
	fprintf(stream, " Time[%f, %f) Bus_wait: %f StreamID: %d\n", start_time, start_time + time_taken, bus_wait_time, streamID);
	return;
}
