	...
	last_erase_time(0.0)
{
    ...
    /* Yoohyuk Lim
     * In SLC_MLC mode, one plane has slc block and mlc block.
     * And each slc and mlc block has the different number of blocks. */
    long physical_address_unit;
    if (SLC_MLC_ENABLE == true)
        physical_address_unit = (long)(PACKAGE_SIZE * DIE_SIZE * PLANE_SIZE * MLC_BLOCK_SIZE);
    else
        physical_address_unit = (long)(PACKAGE_SIZE * DIE_SIZE * PLANE_SIZE * BLOCK_SIZE);

	for (i = 0; i < ssd_size; i++)
	{
		(void) new (&data[i]) Package(*this, bus.get_channel(i), PACKAGE_SIZE, physical_address_unit*i);
	}
	...
	if (PAGE_ENABLE_DATA)
	{
		...
		ulong pageSize = (ulong)SSD_SIZE * (ulong)physical_address_unit * (ulong)PAGE_SIZE;
...
Ssd::~Ssd(void)
{
    ...
    //Yoohyuk Lim
    ulong physical_address_unit;
    if (SLC_MLC_ENABLE == true)
        physical_address_unit = (ulong)(SSD_SIZE * PACKAGE_SIZE * DIE_SIZE * PLANE_SIZE * MLC_BLOCK_SIZE);
    else
        physical_address_unit = (ulong)(SSD_SIZE * PACKAGE_SIZE * DIE_SIZE * PLANE_SIZE * BLOCK_SIZE);
	
	if (PAGE_ENABLE_DATA)
	{
		ulong pageSize = physical_address_unit * (ulong)PAGE_SIZE;
...
{
	return event_arrive(type, logical_address, size, start_time, NULL, STREAMID_DEFAULT);
}

//Yoohyuk Lim
//double Ssd::event_arrive(enum event_type type, ulong logical_address, uint size, double start_time, uint streamID)
//{
//	return event_arrive(type, logical_address, size, start_time, NULL, streamID);
//}
...
{
    return event_arrive(type, logical_address, size, start_time, buffer, STREAMID_DEFAULT);
}

/* Yoohyuk Lim
 * Modified from origin */
double Ssd::event_arrive(enum event_type type, ulong logical_address, uint size, double start_time, void *buffer, uint streamID)
{
    ...
    if (size != 1)
    {
        ...
        for (uint i=0; i<size; i++)
            d += event_arrive(type, logical_address + i, 1, start_time + d, buffer == NULL ? NULL : (char *) buffer + (PAGE_SIZE*i), streamID);

        return d;
    }
    
    ulong physical_address_size = NUMBER_OF_ADDRESSABLE_PAGES;
//    if (SLC_MLC_ENABLE == true)
//        physical_address_size = (ulong)(SSD_SIZE * PACKAGE_SIZE * DIE_SIZE * PLANE_SIZE * MLC_BLOCK_SIZE);
//    else
//        physical_address_size = (ulong)(SSD_SIZE * PACKAGE_SIZE * DIE_SIZE * PLANE_SIZE * BLOCK_SIZE);

	if (VIRTUAL_PAGE_SIZE == 1)
		assert((long long int) logical_address <= (long long int) physical_address_size);
	else
		assert((long long int) logical_address*VIRTUAL_PAGE_SIZE <= (long long int) physical_address_size);

	/* allocate the event and address dynamically so that the allocator can
	 * handle efficiency issues for us */
	Event *event = NULL;

	if((event = new Event(type, logical_address , size, start_time, streamID)) == NULL)
	{
...
void Ssd::write_custom_stat(FILE *stream)
{
	controller.stats.write_custom_stat(stream);
}

// Yoohyuk Lim
void Ssd::print_ftl_statistics(FILE *stream)
{
	controller.print_ftl_statistics(stream);
}
...
/* Yoohyuk Lim */
void Ssd::print_cost_status(FILE *stream)
{
	Block_manager::instance()->print_cost_status(stream);
}
