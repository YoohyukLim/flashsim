...
enum status Plane::write(Event &event)
{
	...
	if(event.get_address().block == next_page.block) {
        ...
        /* Yoohyuk Lim
         * find next page according to streamID
         * But in this simulator, it is meaningless */
...
// Yoohyuk Lim : add streamID to Event
...
enum status Plane::_merge(Event &event)
{
	...
	write.page = 0;
	write.valid = PAGE;
	Event read_event(READ, 0, 1, event.get_start_time(), event.get_streamID());
	Event write_event(WRITE, 0, 1, event.get_start_time(), event.get_streamID());
...
{
    uint block_size = (SLC_MLC_ENABLE == true) ? MLC_BLOCK_SIZE
                                               : BLOCK_SIZE;
	assert(data[address.block].get_pages_valid() < block_size);
    
    long reminder = address.get_linear_address() % block_size;
    
	address.page = data[address.block].get_pages_valid();
	address.valid = PAGE;
	address.set_linear_address(address.get_linear_address()+ address.page - reminder);
	return;
}
...
enum status Plane::get_next_page(block_cell_type ctype)
{
	return SUCCESS; // Yoohyuk Lim: ???
